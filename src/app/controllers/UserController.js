...
class UserController extends Controller {
    ...
    async store (req, res) {
        ...
        if (emailExists) { return res.status(400).json({ error: message('email-used') }) }
    ...
    async update (req, res) {
        ...
        if (!await UserUpdateSchema.isValid(req.body)) { return res.json({ error: message('validation-fails') }) }

        const user = await User.findByPk(req.userId)

        if (!user) { return res.json({ error: message('user-not-exists') }) }
